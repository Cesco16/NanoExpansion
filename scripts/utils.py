...
#            total_length = sum(sections_lengths)#[1:])
#            
#        if ids is None:
#            ids = 'a'
#    
...
#        for length, color in zip(sections_lengths, section_colors):#(sections_lengths[1:], section_colors[1:]):
...
#            if color == 'red':
#                ax.text(x = current_position+length/2, y = 0.01, s = str(length), size=12, ha='center', va='center')
...
#        ax.set_title(f'Expansion in {reg} (dm_26)', fontsize=15) #ax.set_title(f'Expansion in {reg} ({ids})')
#        ax.axis('off')
#    # Mostra il grafico
#        plt.savefig(path + '/' + sample + '/nanoexpansion/' + ids + f'_{str_identifier}.svg')
#        plt.show()
